...
import { layout, symbol, navigation, page as pageIcon, verse } from '@wordpress/icons';
...
import { getEntityActions as _getEntityActions, isEntityReady as _isEntityReady } from '../dataviews/store/private-selectors';
 ...
 */
export const getInsertionPoint = createRegistrySelector(select => createSelector(state => {
...
export const getPostIcon = createRegistrySelector(select => (state, postType, options) => {
  {
    if (postType === 'wp_template_part' || postType === 'wp_template') {
      return __experimentalGetDefaultTemplatePartAreas(state).find(item => options.area === item.area)?.icon || layout;
    }
 ...
 */
export const hasPostMetaChanges = createRegistrySelector(select => (state, postType, postId) => {
  const {
    type: currentPostType,
    id: currentPostId
  } = getCurrentPost(state);
  // If no postType or postId is passed, use the current post.
  const edits = select(coreStore).getEntityRecordNonTransientEdits('postType', postType || currentPostType, postId || currentPostId);
  if (!edits?.meta) {
    return false;
  }

  // Compare if anything apart from `footnotes` has changed.
  const originalPostMeta = select(coreStore).getEntityRecord('postType', postType || currentPostType, postId || currentPostId)?.meta;
 ...
 */
export const getPostBlocksByName = createRegistrySelector(select => createSelector((state, blockNames) => {
