...
exports.compileStyleValue = compileStyleValue;
...
/* Supporting data. */
const ROOT_BLOCK_SELECTOR = exports.ROOT_BLOCK_SELECTOR = 'body';
const ROOT_CSS_PROPERTIES_SELECTOR = exports.ROOT_CSS_PROPERTIES_SELECTOR = ':root';
const PRESET_METADATA = exports.PRESET_METADATA = [{
  ...
  valueFunc: (preset, settings) => (0, _typographyUtils.getTypographyFontSizeValue)(preset, settings),
  valueKey: 'size',
...
const STYLE_PATH_TO_PRESET_BLOCK_ATTRIBUTE = exports.STYLE_PATH_TO_PRESET_BLOCK_ATTRIBUTE = {
...
function getValueFromCustomVariable(features, blockName, variable, path) {
  ...
  const result = (_getValueFromObjectPa = (0, _object.getValueFromObjectPath)(features.settings, ['blocks', blockName, 'custom', ...path])) !== null && _getValueFromObjectPa !== void 0 ? _getValueFromObjectPa : (0, _object.getValueFromObjectPath)(features.settings, ['custom', ...path]);
  if (!result) {
    return variable;
  }
...
function getValueFromVariable(features, blockName, variable) {
  if (!variable || typeof variable !== 'string') {
    if (variable?.ref && typeof variable?.ref === 'string') {
      ...
      variable = (0, _object.getValueFromObjectPath)(features, refPath);
...
function areGlobalStyleConfigsEqual(original, variation) {
  ...
  return (0, _es.default)(original?.styles, variation?.styles) && (0, _es.default)(original?.settings, variation?.settings);
}
...
function getResolvedRefValue(ruleValue, tree) {
  ...
  if (typeof ruleValue !== 'string' && ruleValue?.ref) {
    ...
    const resolvedRuleValue = compileStyleValue((0, _object.getValueFromObjectPath)(tree, refPath));
