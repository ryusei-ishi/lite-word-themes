  ...
  const output = Object.entries(_blocks.__EXPERIMENTAL_STYLE_PROPERTY).reduce((declarations, [key, {
    ...
    if (properties && typeof styleValue !== 'string') {
      Object.entries(properties).forEach(entry => {
        ...
        declarations.push(`${cssProperty}: ${(0, _utils.compileStyleValue)((0, _object.getValueFromObjectPath)(styleValue, [prop]))}`);
      });
    } else if ((0, _object.getValueFromObjectPath)(blockStyles, pathToValue, false)) {
      const cssProperty = key.startsWith('--') ? key : kebabCase(key);
      declarations.push(`${cssProperty}: ${(0, _utils.compileStyleValue)((0, _object.getValueFromObjectPath)(blockStyles, pathToValue))}`);
    }
    return declarations;
  }, []);
