 ...
 */
import { WebrtcProvider, SignalingConn, WebrtcConn, signalingConns, rooms, publishSignalingMessage, log } from './y-webrtc/y-webrtc';
            ...
            switch (data.type) {
              case 'announce':
                if (webrtcConns.size < room.provider.maxConns) {
                  map.setIfUndefined(webrtcConns, data.from, () => new WebrtcConn(signalCon, true, data.from, room));
                  emitPeerChange();
                }
                break;
              case 'signal':
                ...
                if (data.to === peerId) {
                  map.setIfUndefined(webrtcConns, data.from, () => new WebrtcConn(signalCon, false, data.from, room)).peer.signal(data.signal);
                  emitPeerChange();
                }
                break;
            }
    ...
    if (httpClient.ws) {
      ...
      httpClient.ws.send = function send( /** @type {string} */message) {
        window.fetch(url, {
          ...
          log('Error sending to server with message: ' + message);
        });
      };
    }
    eventSource.onerror = () => {
...
export class WebrtcProviderWithHttpSignaling extends WebrtcProvider {
  connect() {
    ...
    this.signalingUrls.forEach(( /** @type {string} */url) => {
      const signalingConn = map.setIfUndefined(signalingConns, url,
