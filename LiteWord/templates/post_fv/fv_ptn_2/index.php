...
// CSSの読み込み
wp_enqueue_style('post_fv_2_style', get_template_directory_uri() . '/templates/post_fv/fv_ptn_2/style.min.css', array(), css_version(), 'all');

// インラインスタイルの追加
$custom_css = "
    .post_fv_ptn_2 .image_thumbnail::after {
...
<header class="post_fv_ptn_2">
    ...
    <?php else: ?>
    <div class="ttl_wrap_out">
        <div class="post_data">
            <?php
                $fv_date = Lw_theme_mod_set("single_post_layout_fv_date", "active");
                if($fv_date == "active"):
            ?>
                <time class="post_date_time" datetime="<?=get_the_date('Y-m-d')?>" itemprop="datePublished"><?=get_the_date('Y.m.d')?></time>
            <?php endif; ?>
            <?php
                $fv_category = Lw_theme_mod_set("single_post_layout_fv_category", "active");
                if($fv_category == "active"):
