 ...
 *
 * 会員サイトの「会員」に WordPress の管理画面を見せる理由は無いので、
 * 会員サイトとして使うならONにしてもらう。
 *
 * 🚨 既定はOFF。テーマを更新しただけの既存サイトの挙動を変えないため。
 *    購読者を会員以外の用途で使っているサイト（WooCommerce の customer、
 *    コメント購読、スタッフ用アカウント等）がある。そこで黙ってONになると、
 *    その人たちが何の知らせもなく管理画面に入れなくなる。
 *
 ...
 * この機能を使うか（既定OFF）
 *
 * 未設定（'' ＝「入れる（既定）」）と 'off' はどちらも「締め出さない」。
 * カスタマイザーで明示的にONにしたときだけ効く。
 *
 * @return bool
 */
function lw_membership_lock_admin_enabled() {
	return Lw_theme_mod_set( 'lw_membership_lock_admin', '' ) === 'on';
}
