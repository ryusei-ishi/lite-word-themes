...
function lw_seo_bulk_edit_page() {
	...
	if ( ! $current_type ) : ?>
		...
		<div class="lw-seo-type-select">
			<div class="lw-seo-type-header">
				...
				<?php lw_voice_help_button( 'lw-seo-bulk-guide' ); ?>
			</div>
			<p class="lw-seo-type-desc">編集したい投稿タイプを選択してください。</p>
			<?php lw_voice_help_text( 'lw-seo-bulk-guide', lw_get_seo_bulk_guide_text() ); ?>
	 ...
	 * (4) 一覧出力
	 *──────────────────────────────────────────*/ ?>
	<div class="lw-seo-bulk-wrap">
		<!-- ページヘッダー -->
		<div class="lw-seo-bulk-header">
			<div class="lw-seo-bulk-header-left">
				<span class="dashicons dashicons-<?php echo $current_type === 'post' ? 'admin-post' : 'admin-page'; ?>"></span>
				<div class="lw-seo-bulk-header-text">
					<h1><?php echo esc_html( $page_title ); ?></h1>
					<span class="lw-seo-bulk-count"><?php echo number_format( $total_posts ); ?> 件</span>
				</div>
				<?php lw_voice_help_button( 'lw-seo-bulk-guide-list' ); ?>
			</div>
		</div>
		<?php lw_voice_help_text( 'lw-seo-bulk-guide-list', lw_get_seo_bulk_guide_text() ); ?>

		<!-- フィルターバー -->
		<div class="lw-seo-filter-bar">
			<!-- 投稿タイプ切替 -->
			<div class="lw-seo-filter-group">
				<span class="lw-seo-filter-label">表示</span>
				<div class="lw-seo-filter-tabs">
					<a href="<?php echo esc_url( add_query_arg(['type'=>'post','cat'=>null,'sort'=>$sort_param,'show_posts'=>$show_posts?'1':'0'], $base_admin) ); ?>" class="lw-seo-filter-tab <?php echo $current_type==='post'?'active':''; ?>">
						<span class="dashicons dashicons-admin-post"></span> 投稿
					</a>
					<a href="<?php echo esc_url( add_query_arg(['type'=>'page','parent'=>null,'sort'=>$sort_param], $base_admin) ); ?>" class="lw-seo-filter-tab <?php echo $current_type==='page'?'active':''; ?>">
						<span class="dashicons dashicons-admin-page"></span> 固定ページ
					</a>
				</div>
			</div>
			...
			<!-- 並び替え -->
			<div class="lw-seo-filter-group">
				<span class="lw-seo-filter-label">並び替え</span>
				<div class="lw-seo-sort-tabs">
					<?php foreach ( $sort_opts as $key => $opt ) :
						$url_params = ['type'=>$current_type,'sort'=>$key];
						if ( $current_type === 'post' ) {
							$url_params['cat'] = $cat_filter ?: null;
							$url_params['show_posts'] = $show_posts ? '1' : '0';
						...
						$url = add_query_arg( $url_params, $base_admin );
					?>
						<a href="<?php echo esc_url($url); ?>" class="lw-seo-sort-tab <?php echo $key === $sort_param ? 'active' : ''; ?>"><?php echo esc_html($opt['label']); ?></a>
					<?php endforeach; ?>
				</div>
			</div>
		</div>
		...
		<?php if ( $current_type === 'post' ) : ?>
			<div class="lw-seo-category-filter">
				<div class="lw-seo-category-header">
					<span class="dashicons dashicons-category"></span>
					<span class="lw-seo-category-title">カテゴリー</span>
					<a href="<?php echo $toggle_url; ?>" class="lw-seo-toggle-posts"><?php echo esc_html( $toggle_label ); ?></a>
				</div>
				<div class="lw-cat-list_wrap reset"><?php echo $cat_list; ?></div>
			</div>
		<?php else : ?>
			<div class="lw-seo-category-filter">
				<div class="lw-seo-category-header">
					<span class="dashicons dashicons-admin-page"></span>
					<span class="lw-seo-category-title">ページ階層</span>
				</div>
				<div class="lw-cat-list_wrap reset"><?php echo $page_list; ?></div>
			</div>
		<?php endif; ?>
	...
	.lw-seo-bulk-header-left {
		display: flex;
		align-items: center;
		gap: 16px;
	}
	.lw-seo-bulk-header-left > .dashicons {
		display: flex;
		align-items: center;
		justify-content: center;
		width: 56px;
	...
	.lw-seo-filter-tab .dashicons {
		font-size: 16px;
		width: 16px;
		height: 16px;
	}
	.lw-seo-filter-tab:hover {
		background: #e2e8f0;
		color: #1e293b;
	}
	.lw-seo-filter-tab.active {
		background: linear-gradient(135deg, #2b72b5 0%, #3d8fd1 100%);
		color: #fff;
		box-shadow: 0 2px 8px rgba(43, 114, 181, 0.3);
	}
	.lw-seo-sort-tabs {
		display: flex;
		gap: 4px;
	...
	.lw-seo-sort-tab:hover {
		background: #f1f5f9;
		color: #1e293b;
	}
	.lw-seo-sort-tab.active {
		background: #1e3a5f;
		color: #fff;
	}

	/* カテゴリーフィルター */
	.lw-seo-category-filter {
		margin-bottom: 20px;
		background: #fff;
		border-radius: 12px;
		box-shadow: 0 2px 8px rgba(0, 0, 0, 0.04);
	...
	.lw-seo-category-header .dashicons {
		font-size: 18px;
		width: 18px;
		height: 18px;
		color: #2b72b5;
	}
	.lw-seo-category-title {
		font-size: 14px;
		font-weight: 600;
		color: #1e3a5f;
		flex: 1;
	...
	.lw-seo-toggle-posts:hover {
		text-decoration: underline;
	}
	.lw-seo-category-filter .lw-cat-list_wrap {
		margin: 0;
		padding: 16px 20px;
		max-height: 250px;
		border-radius: 0;
