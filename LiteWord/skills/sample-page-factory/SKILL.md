...
## 6. 商品を紹介するページ・アフィリエイトのページを作るとき（2026-09-02 追加）

### 使えるブロック3つ

| ブロック | 何が並ぶか |
|---|---|
| `wdl/lw-pr-product-1` | 商品リンクの箱。写真・商品名・ひとこと・参考価格＋Amazon/楽天/Yahoo!など複数店舗のボタン |
| `wdl/lw-pr-ranking-1` | ランキング。順位の札・写真・評価の星・良い点／気になる点・店舗ボタンを縦に並べる |
| `wdl/lw-pr-goodbad-1` | メリットとデメリットを左右2つの箱に |

属性は `my-blocks/build/<ブロック名>/block.json` を開いて確かめること（**手で推測しない**）。
`lw-pr-ranking-1` は38個、`lw-pr-product-1` は31個、`lw-pr-goodbad-1` は23個あります。

### 広告リンクの印は、ここでは markup に直接書ける

記事をマークダウンで書く場合（`skills/blog-writer/SKILL.md`）は rel を付ける記法が無く、
貼り付けたあとに書式ボタンで付けます。**ページを markup で作るこの手順では最初から書けます。**

```
<a href="https://example.com/" rel="sponsored nofollow" target="_blank">…</a>
```

⚠️ ブロックの属性側にも同じリンクが入っている場合は、**属性と HTML の両方**を直すこと
（片方だけだとエディタで「ブロックが壊れています」になる → 3章）。

### PR表記は markup に書かない

「この記事には広告が含まれます」の帯は、**カスタマイザーの PR表記**が出します
（外観 → カスタマイズ → PR表記。無料）。ページの本文に手で書くと、設定と二重になります。

### 手本にできる本番のページ

lite-word.com に**業種まるごとのサンプルが8業種54ページ**あります。うちアフィリエイトの型は
`https://lite-word.com/cosme-top/`（コスメ比較・レビュー6ページ）。
PR表記の帯・比較表・ランキング・レビュー・運営者情報・広告についてまで1式そろっています。

作り方の道具は `knowledge/projects/liteword-promotion/scripts/_lib/sample-site.js`（全業種共通）。
業種側は `<業種>/sample-site-def.js` に値だけ書き、`node <業種>/run-sample-site.js --go` で本番へ入ります。
🚨 **業種ごとにスクリプトをコピーしないこと**（検算がコピーした時点で止まる）。
